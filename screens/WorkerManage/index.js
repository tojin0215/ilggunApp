...
const WorkerManageScreen = ({ navigation, route }) => {
				...
				[
					...
					},
					{
						...
						onPress: () => {
							...
							axios
								.post(
									'http://13.124.141.28:3000/deleteWorker',
									{
										business: bangCode,
										workername: name,
									},
	...
	async function fetchData(bangCode) {
		try {
			await axios
				.post(
					'http://13.124.141.28:3000/selectWorkerByType',
					{
						business: bangCode,
						type: 2,
					},
			...
			<View style={styles.container}>
				<View style={styles.buttonArea}>
					<TouchableOpacity
						style={styles.button}
						onPress={() =>
							navigation.navigate('Invite', { type: 2, user_id: idid })
						}
					>
						<Image
					...
					{business.map((b, id) => (
						<View style={styles.worker} key={id}>
							...
							<Text style={styles.textTitle2}>
								({b.workername.substr(0, 4)}
								{b.workername.length > 4 ? '**' : ''})
							</Text>
							<View style={styles.ContbuttonArea}>
								<TouchableOpacity
									...
									onPress={() => {
										if (b.state != 0) {
											navigation.navigate('Contract FormA', {
								...
								>
									{b.state == 0 ? (
										...
										<Image
											...
											source={require('../../img/contractImg.png')}
										/>
									)}
								</TouchableOpacity>
							</View>
