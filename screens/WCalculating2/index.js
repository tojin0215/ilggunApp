...
class WCalculatingScreen2 extends Component {
	state = {
		JoinYear: 0,
		JoinMonth: 0,
		JoinDay: 0, //입사 년,월,일
		...
		term3BasePay: null,
		term4BasePay: null,
		term1OtherAllowance: null,
		term2OtherAllowance: null,
		term3OtherAllowance: null,
		term4OtherAllowance: null,
		diff3Month: 0,
	...
	updateState() {
		const Join = new Date(
			this.state.JoinYear,
			this.state.JoinMonth - 1,
			this.state.JoinDay
		);
		const Leave = new Date(
			this.state.LeaveYear,
			this.state.LeaveMonth - 1,
			this.state.LeaveDay
		);
			...
			this.setState({
				...
				tableData: [
					[
						diff3,
						<TextInput
							value={this.state.term3BasePay}
							keyboardType={'number-pad'}
							onChangeText={(term3BasePay) => this.setState({ term3BasePay })}
							placeholder={'1000000'}
							ref={(input) => {
								this.TextInput11 = input;
							}}
							onSubmitEditing={() => {
						...
						/>,
						<TextInput
							value={this.state.term3OtherAllowance}
							keyboardType={'number-pad'}
							onChangeText={(term3OtherAllowance) =>
								this.setState({ term3OtherAllowance })
							}
							placeholder={'20000'}
							ref={(input) => {
								this.TextInput12 = input;
							}}
							onSubmitEditing={() => {
					...
					[
						diff2,
						<TextInput
							value={this.state.term2BasePay}
							keyboardType={'number-pad'}
							onChangeText={(term2BasePay) => this.setState({ term2BasePay })}
							placeholder={'1000000'}
							ref={(input) => {
								this.TextInput13 = input;
							}}
							onSubmitEditing={() => {
						...
						/>,
						<TextInput
							value={this.state.term2OtherAllowance}
							keyboardType={'number-pad'}
							onChangeText={(term2OtherAllowance) =>
								this.setState({ term2OtherAllowance })
							}
							placeholder={'20000'}
							ref={(input) => {
								this.TextInput14 = input;
							}}
							onSubmitEditing={() => {
					...
					[
						diff1,
						<TextInput
							value={this.state.term1BasePay}
							keyboardType={'number-pad'}
							onChangeText={(term1BasePay) => this.setState({ term1BasePay })}
							placeholder={'1000000'}
							ref={(input) => {
								this.TextInput15 = input;
							}}
							onSubmitEditing={() => {
						...
						/>,
						<TextInput
							value={this.state.term1OtherAllowance}
							keyboardType={'number-pad'}
							onChangeText={(term1OtherAllowance) =>
								this.setState({ term1OtherAllowance })
							}
							placeholder={'20000'}
							ref={(input) => {
								this.TextInput16 = input;
							}}
							style={styles.input}
						/>,
					],
					...
					[
						diff1 + diff2 + diff3,
						parseInt(this.state.term3BasePay) +
							parseInt(this.state.term2BasePay) +
							parseInt(this.state.term1BasePay),
						parseInt(this.state.term3OtherAllowance) +
							parseInt(this.state.term2OtherAllowance) +
							parseInt(this.state.term1OtherAllowance),
					],
				],
				term4BasePay: 0,
				term4OtherAllowance: 0,
				diff4: 0,
			...
			this.setState({
				...
				tableData: [
					[
						diff4,
						<TextInput
							value={this.state.term4BasePay}
							keyboardType={'number-pad'}
							onChangeText={(term4BasePay) => this.setState({ term4BasePay })}
							placeholder={'1000000'}
							ref={(input) => {
								this.TextInput11 = input;
							}}
						...
						/>,
						<TextInput
							value={this.state.term4OtherAllowance}
							keyboardType={'number-pad'}
							onChangeText={(term4OtherAllowance) =>
								this.setState({ term4OtherAllowance })
							}
							placeholder={'200000'}
							ref={(input) => {
								this.TextInput12 = input;
					...
					[
						...
						<TextInput
							...
							placeholder={'1000000'}
							ref={(input) => {
								this.TextInput17 = input;
							}}
							onSubmitEditing={() => {
								this.TextInput18.focus();
							}}
							blurOnSubmit={false}
							style={styles.input}
						/>,
					...
					[
						...
						parseInt(this.state.term4BasePay) +
							parseInt(this.state.term3BasePay) +
							parseInt(this.state.term2BasePay) +
							parseInt(this.state.term1BasePay),
						parseInt(this.state.term4OtherAllowance) +
							parseInt(this.state.term3OtherAllowance) +
							parseInt(this.state.term2OtherAllowance) +
							parseInt(this.state.term1OtherAllowance),
					],
	...
	resetData() {
		this.setState({
			...
			term4OtherAllowance: 0,
			term3OtherAllowance: 0,
			term2OtherAllowance: 0,
			term1OtherAllowance: 0,
			term4BasePay: 0,
			term3BasePay: 0,
			term2BasePay: 0,
			term1BasePay: 0,
	...
	SeverancePayCalculation() {
		const sumTermBasePay =
			parseInt(this.state.term1BasePay) +
			parseInt(this.state.term2BasePay) +
			parseInt(this.state.term3BasePay) +
			parseInt(this.state.term4BasePay);
		const sumTermOtherAllowance =
			parseInt(this.state.term1OtherAllowance) +
			parseInt(this.state.term2OtherAllowance) +
			parseInt(this.state.term3OtherAllowance) +
			parseInt(this.state.term4OtherAllowance);
		...
		const AverageDailyWage1 = (
			(TotalWage3Month + AnnualBonus1 + AnnualAllowance1) /
			this.state.diff3Month
		).toFixed(2); //1일평균임금=3개월지급총액(A+B+C)/3개월 총 일수
		const SeverancePay1 = (
			AverageDailyWage1 *
			30 *
			(this.state.NumberOfWorkingDays / 365)
		).toFixed(2); //퇴직금=1일평균임금*30일*(재직일수/365)

		this.setState({
			AverageDailyWage: parseInt(AverageDailyWage1)
				.toString()
				.replace(/\B(?=(\d{3})+(?!\d))/g, ','),
			SeverancePay: parseInt(SeverancePay1)
				.toString()
				.replace(/\B(?=(\d{3})+(?!\d))/g, ','),
		});
	}
		...
		return (
            ...
            <Text>퇴사일자 : {this.state.DateOfResignation}</Text>  */}
							<Text style={styles.textStyle1}>
								재직일수 : {this.state.NumberOfWorkingDays}일
							</Text>
						</View>
						...
						<View style={styles.textArea}>
							...
							<View style={styles.rowView}>
								...
								<TextInput
									...
									ref={(input) => {
										this.TextInput30 = input;
									}}
									onSubmitEditing={() => {
										this.TextInput31.focus();
									}}
									blurOnSubmit={false}
							...
							<View style={styles.rowView}>
								...
								<TextInput
									...
									onChangeText={(AnnualAllowance) =>
										this.setState({ AnnualAllowance })
									}
									ref={(input) => {
										this.TextInput31 = input;
									}}
									blurOnSubmit={false}
									placeholder={'연차수당'}
									keyboardType={'number-pad'}
						...
						<View style={styles.resultArea}>
							<Text style={styles.textStyle1}>
								* 기본급/기타수당 입력하기 버튼을 누른 후 퇴직금 계산하기 버튼을
								누르시오.
							</Text>
							<TouchableOpacity
								style={styles.button}
								onPress={() => {
									this.SeverancePayCalculation();
								}}
							>
						...
						<View style={{ marginTop: hp('2%') }}>
							<Text style={styles.textStyle}>
								* 회사 내규 등에따라 실제 지급액과가 차이가 있을 수 있습니다.
							</Text>
						</View>
					</ScrollView>
				</View>
