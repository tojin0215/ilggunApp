...
class ExpenseScreen2 extends Component {
	...
	updateState() {
		...
		this.setState({
			Salary: parseInt(Salary1)
				.toString()
				.replace(/\B(?=(\d{3})+(?!\d))/g, ','),
			WithholdingTax: parseInt(WithholdingTax1)
				.toString()
				.replace(/\B(?=(\d{3})+(?!\d))/g, ','),
			RealSalary: parseInt(RealSalary1)
				.toString()
				.replace(/\B(?=(\d{3})+(?!\d))/g, ','),
		});
	}
	...
	render() {
		const {
			selectedHoursGoToWork,
			selectedMinutesGoToWork,
			selectedHoursOffWork,
			selectedMinutesOffWork,
			HourlyWage,
			WithholdingTax,
			Salary,
			RealSalary,
				...
				<View style={styles.container}>
					<ScrollView>
						<View style={styles.titleArea}>
							<Text style={styles.textTitle}>인건비 계산하기(비정규직)</Text>
						</View>
						...
						<View style={{ marginTop: hp('2%') }}>
							<Text style={styles.textStyle}>근무시간 설정</Text>
						</View>
										...
										<View style={styles.pickerArea}>
											<CheckboxGroup
												...
												checkboxes={[
													{
														label: (
															<Text style={styles.listText}>{item.week}</Text>
														),
                                ...
                                <Text style={styles.item} 
                                    onPress={this.getListViewItem.bind(this, item)}>{item.week}</Text> 
											...
											<Text style={styles.listText}>
												출근시간) {item.goToWorkHour}시 : {item.goToWorkMin}분
											</Text>
											<TimePicker
												key={item.key}
											...
											/>
											<Text style={styles.listText}>
												퇴근시간) {item.offWorkHour}시 : {item.offWorkMin}분
											</Text>
											<TimePicker
