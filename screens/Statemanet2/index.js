				...
				.then((res) => {
					for (let i = 0; i < res.data.length; i++) {
						...
						if (res.data[i].date === this.state.itemAA.split('년')[0]) {
							const NationalPensionPercentage = res.data[i]
								.NationalPensionPercentage
								? res.data[i].NationalPensionPercentage
								: 4.5;
							const HealthInsurancePercentage = res.data[i]
								.HealthInsurancePercentage
								? res.data[i].HealthInsurancePercentage
								: 3.43;
							const RegularCarePercentage = res.data[i].RegularCarePercentage
								? res.data[i].RegularCarePercentage
								: 11.25;
							const EmploymentInsurancePercentage = res.data[i]
								.EmploymentInsurancePercentage
								? res.data[i].EmploymentInsurancePercentage
								: 0.8;
							const pay11 = res.data[i].HourlyWage
								? res.data[i].HourlyWage
								: 8720;
							const HourlyWage = res.data[i].HourlyWage
								? res.data[i].HourlyWage
								: 8720;
							...
							console.log('고용보험', EmploymentInsurancePercentage);
								...
								if (
									...
									if (type_alba) {
										...
										if (
											...
											for (let j = 0; j < (worker_start_day - 1) % 7; j++) {
												weekk[dd]++;
												dd++;
												dd = dd % 7;
											}
