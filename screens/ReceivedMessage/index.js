...
const ReceivedMessageScreen = ({ navigation, route }) => {
	...
	async function fetchData(idid) {
		try {
			await axios
				.post(
					'http://13.124.141.28:3000/selectReceivedMessage',
					{
						t: idid,
						ft: 0,
					},
			...
			.then(async (res) => {
				if (res.data[0] === undefined) {
					...
					try {
						...
						let dd = d.split('-');
						console.log(
							work + '/' + busi + '/' + d + '/' + dayOfWeek + '/' + dd
						);
						let ori = 0;
						axios
							.post(
								'http://13.124.141.28:3000/selectWorkerEach',
								{
									business: busi,
									workername: work,
								},
                  ...
                  body: JSON.stringify({
                    ...
                    workername: work
							...
							.then((res) => {
								...
								axios.post(
									'http://13.124.141.28:3000/addWork',
									{
										business: busi,
										workername: work,
										month: dd[1] * 1,
						...
						if (t == 2) {
							try {
								axios
									.post(
										'http://13.124.141.28:3000/sendMessage',
										{
											f: id,
											message:
												'<' +
	...
	async function alterRead() {
		try {
			axios
				.post(
					'http://13.124.141.28:3000/alterReadMessage',
					{
						ind: index,
					},
					{
			...
			axios
				...
				.then(async (res) => {
					if (res.data[0] === undefined) {
						...
						axios
							.post(
								'http://13.124.141.28:3000/addBang',
								{
									bang: busi,
									id: id,
								},
						...
						axios
							.post(
								'http://13.124.141.28:3000/alterState',
								{
									bang: busi,
									type: 1,
									id: id,
	...
	function delMessage(i) {
		Alert.alert('메세지 삭제', '메세지를 삭제하시겠습니까?', [
			{
				text: 'OK',
				onPress: () => {
					try {
						axios
							.post(
								'http://13.124.141.28:3000/delMessage',
								{
									ind: i,
								},
								{
		...
		<View style={styles.image}>
			<View style={styles.container}>
						...
						{business.map((b, id) => (
							<View style={b.r ? styles.readMessageArea : styles.messageArea}>
								<Image
									...
									source={require('../../img/user_blue.png')}
								/>
								<TouchableOpacity
				...
				<View style={styles.sendBtnArea}>
					<TouchableOpacity onPress={() => navigation.navigate('Send Message')}>
						<Image
