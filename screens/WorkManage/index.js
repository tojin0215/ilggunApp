...
const WorkManageScreen = ({ navigation, route }) => {
	...
	async function fetchData(bangCode, date) {
		try {
			...
			await axios
				.post(
					'http://13.124.141.28:3000/selectWorkerAsDay',
					{
						business: bangCode,
						year: d[3] * 1,
						month: d[1] * 1,
          ...
          body: JSON.stringify({
				...
				.then(async (res) => {
					...
					for (let i = 0; i < resultData.length; i++) {
						...
						if (resultData[i].startdate !== null) {
							...
							if (
								(d[3] * 1 == dd[0] && d[1] * 1 == dd[1] && d[2] * 1 >= dd[2]) ||
			...
			await axios
				.post(
					'http://13.124.141.28:3000/selectTimelog',
					{
						bang: bangCode,
						year: d[3] * 1,
						month: d[1] * 1,
			...
			<View style={styles.container}>
				<View style={styles.calenderArea}>
					...
					>
						...
						<Text style={styles.calenderTitle}>
							{route.params.selecteddate.split(' ')[3]}년{' '}
							{route.params.selecteddate.split(' ')[1]}월{' '}
							{route.params.selecteddate.split(' ')[2]}일(
							{day(route.params.selecteddate.split(' ')[0])})
						</Text>
				...
				<View style={styles.listArea}>
					<ScrollView>
						{String(route.params.selecteddate).split(' ')[0] === 'Mon'
							? business.map((b, id) => (
									<View style={styles.workerAera}>
										<Image
											...
											source={require('../../img/user_mint.png')}
										/>
										<Text
											key={id}
											numberOfLines={1}
										...
										<View style={styles.colStyle}>
											...
											>
												<Text key={id} style={styles.werkerTextTime}>
													{b.time != undefined || b.time != null
														? b.time.slice(0, 2) +
														  ':' +
														  b.time.slice(2, 4) +
														  ' ~ ' +
											...
											<View style={{ flexDirection: 'row' }}>
												<Text key={id} style={styles.werkerTextTime}>
													{timelog[b.workername] == undefined ||
													timelog[b.workername] == null
														? '출근'
														: timelog[b.workername].slice(0, 1) == ''
														? '출근'
														: timelog[b.workername].slice(0, 2) +
														  ':' +
														  timelog[b.workername].slice(2, 4)}
												</Text>
												...
												<Text key={id} style={styles.werkerTextTime}>
													...
													timelog[b.workername] == null
														? '퇴근'
														: timelog[b.workername].slice(4, 5) == ''
														? '퇴근'
														: timelog[b.workername].slice(4, 6) +
														  ':' +
														  timelog[b.workername].slice(6, 8)}
												</Text>
											</View>
										</View>
										<View style={styles.listStyle}>
											<TouchableOpacity
												...
												onPress={() =>
													navigation.navigate('Add WorkTodo', {
							...
							: business.map((b, id) => <Text key={id}>{b.workername}</Text>)}
					</ScrollView>
				</View>
				...
				<View style={styles.buttonArea}>
					<TouchableOpacity
						style={styles.button}
						onPress={() =>
							navigation.navigate('alter worker', {
								date: route.params.selecteddate,
							})
						}
					>
